...
- 重建期间旧服务仍可能返回成功。用本次启动标识、构建版本或新行为探针确认接管完成；磁盘上的新包和一个 HTTP 200 不能证明测试运行在新代码上。
...
选择能区分正确与错误实现的少量样本，比增加同质用例更有价值：

- **输入来源一致**：至少一条链路从真实页面或正式导入入口准备数据；直接写库的理想数据可能绕过名称/标识、空值和序列化差异。
- **身份和范围可区分**：验证人员分配时用不同办理人及一个无权用户；验证筛选或批量操作时同时放命中与未命中记录，断言后者未被返回或修改。全程管理员或全部字段同一人无法证明授权与分配正确。
- **配置走到结果**：保存回显只是第一步，抽一项走完配置、发布、运行和用户可见结果；配置存在但变量未注入、适配器未接入或审计未展示，都属于链路缺口。
- **跨动作交错**：涉及共享数据，选择一组旧草稿、后续写入、撤销或退回与自动跳过的组合；确认保护在组合后仍有效。顺序测试通过不能替代并发实测。

失败响应、字段缺失、`null` 与明确的 `false` 分开断言；否则接口故障会被伪装成“无人可选”或“状态正常”。共用环境的测试记录标记归属，只清理本轮创建且获准清理的数据，保留用户验收样本。
