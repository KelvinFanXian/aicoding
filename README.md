...
> 当我说“参考 aicoding”时，先在当前环境找 aicoding 仓库；没有就先执行 `mkdir -p ~/github`，再执行 `git clone git@github.com:KelvinFanXian/aicoding.git ~/github/aicoding`；已有且工作区干净就执行 `git -C ~/github/aicoding pull`。然后读仓库根目录的 AGENTS.md，按当前任务选读相关 Skill 并应用。它默认只读，以当前项目事实为准；只有我明确要求维护 aicoding 时才写入。克隆或拉取失败时说明原因与当前可用版本，不要假装已更新。

这里就是 aicoding 的维护工作区；用户明确要求在本仓库写入时，直接按维护模式处理。`~/github/aicoding` 是**当前环境**用户目录下的约定位置，云端与异机各自独立；克隆前确认父目录存在，已有副本存在本地改动时不要强行拉取。

访问权限采用安全默认值：

- **只读使用（默认）**：引用本仓库辅助其他工作时，可以获取或更新副本，除此之外只能读取和应用，不能修改本仓库内容。
